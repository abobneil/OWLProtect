...
2. Authenticate with the existing bootstrap admin session and change the password through `/admins/default/password`.
...
5. Verify that `/bootstrap` still reports password rotation and MFA state correctly for the seeded admin account.
...
- Rotate provider secrets by updating the mounted secret, restarting control-plane instances, and verifying `/auth/provider/login` against each configured provider.
