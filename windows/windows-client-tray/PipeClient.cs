...
internal static class PipeClient
{
    ...
    public static async Task<PipeResponse> RequestStatusAsync(CancellationToken cancellationToken = default) =>
        await SendCommandAsync(PipeProtocol.StatusCommand, cancellationToken);

    public static async Task<PipeResponse> SendCommandAsync(string command, CancellationToken cancellationToken = default)
    {
        ...
        var request = new PipeRequest(PipeProtocol.Version, Guid.NewGuid().ToString("n"), command, true);
